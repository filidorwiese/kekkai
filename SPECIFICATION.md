...
Each image additionally carries label `kekkai.config_hash = sha256(debian base + "\n" + nvm tag + "\n" + node_version selector + "\n" + builtin+user apt_packages + "\n" + embed/init-firewall.sh + "\n" + uid:gid)[:12]` — the bake inputs *minus* the claude version. It exists solely so the §6.2 offline fallback can find images built for this config; it never keys builds. Identity is part of it so the fallback never reuses an image baked for a different host user.
