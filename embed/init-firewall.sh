...
[kekkai] ****************************************************************
EOF
    # Observe-only NFLOG for `kekkai traffic` (§9): policies stay ACCEPT, no
...
# NFLOG group-1 DNS taps for `kekkai traffic` (§9): non-terminating, verdicts
...
# Observe-only NFLOG mirror of the bridge ACCEPT below (`kekkai traffic`, §9).
...
# NFLOG taps for `kekkai traffic` (§9): each log rule sits immediately before
